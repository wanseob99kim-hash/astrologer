 ...
 */
export function ScoreBoard({ kootas, pointsLabel }: { kootas: readonly KootaScore[]; pointsLabel: (n: number) => string }) {
  return (
      ...
      {kootas.map((entry) => (
        <li key={entry.koota.key} className={entry.isWeak ? 'is-weak' : undefined}>
          <span className="kootaBoard__name">
            {entry.koota.ko}
            <span className="kootaBoard__max">{pointsLabel(entry.maxScore)}</span>
          </span>
