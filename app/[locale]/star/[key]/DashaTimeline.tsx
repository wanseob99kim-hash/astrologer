 ...
 */
export function DashaTimeline({ timeline, locale }: { timeline: readonly DashaPeriod[]; locale: Locale }) {
      ...
      >
        {timeline.map((period) => (
          <div
            ...
            title={t.dashaTitle(period.planetKo, Math.round(period.startAge), Math.round(period.endAge))}
            style={{
        ...
        {timeline.map((period) => (
          ...
          >
            <span style={{ color: 'var(--muted)' }}>
              {t.dashaRange(Math.round(period.startAge), Math.round(period.endAge))}
            </span>
            <span>{period.planetKo}</span>
            {period.isCurrent ? <span style={{ color: 'var(--marigold)' }}>{t.dashaNow}</span> : <span />}
          </li>
        ))}
      </ol>
    </div>
